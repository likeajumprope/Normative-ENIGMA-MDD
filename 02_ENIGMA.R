...
# remove cortical thickness values that are impossible
...
if(write){
  write.csv(ENIGMA_thick_cov, "ENIGMA_thick_cov.csv", row.names = FALSE)
}
...
## Sensitivity analysis: imputing MDD test set with itself, compared to with healthy controls training set

set.seed(1)
preProcValues_MDD <-
  preProcess(test_MDD[, c(1:35, 39)], method = c("knnImpute"), k = 10)
imp_test_MDD_by_MDD <- test_MDD
imp_test_MDD_by_MDD[, c(1:35, 39)] <-
  predict(preProcValues_MDD, test_MDD[, c(1:35, 39)])

vis_dat(test_MDD[, c(1:35, 39)])

p_values <- list()
mean_of_imp_test_MDD <- list()
mean_of_imp_test_MDD_by_MDD <- list()
for (i in 1:35){
  print(t.test(imp_test_MDD[i], imp_test_MDD_by_MDD[i]))
  coef <- t.test(imp_test_MDD[i], imp_test_MDD_by_MDD[i])
  p_values <- append(p_values, coef$p.value)
  mean_x <- as.numeric(coef$estimate[1])
  mean_of_imp_test_MDD <- append(mean_of_imp_test_MDD, as.numeric(coef$estimate[1]))
  mean_of_imp_test_MDD_by_MDD <- append(mean_of_imp_test_MDD_by_MDD, as.numeric(coef$estimate[2]))
}

tmp.fdr.p= data.frame(p_vales=c(p.adjust(p_values[1:35],method="fdr")),
                      regions = c(colnames(imp_test_MDD_by_MDD)[1:35]),
                      mean_of_imp_test_MDD = c(unlist(mean_of_imp_test_MDD)),
                      mean_of_imp_test_MDD_by_MDD = c(unlist(mean_of_imp_test_MDD_by_MDD))
                      )

tmp.fdr.p[, sapply(tmp.fdr.p, is.numeric)] <- apply(tmp.fdr.p[, sapply(tmp.fdr.p, is.numeric)], 2, round, digits = 4)

if(write){
  write.csv(tmp.fdr.p, "submission/sensitivity_analysis.csv")
}
